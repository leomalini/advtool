-- ============================================================
-- 20 — REPARO da migration 07 (events_enhanced)
--
-- A 07 consta como aplicada no histórico, mas seu conteúdo nunca chegou a
-- rodar no banco: faltavam as 12 colunas novas de `events` e as tabelas
-- `event_assignees` / `event_attachments`. O sintoma era o PostgREST
-- respondendo PGRST200 ("Could not find a relationship between 'events' and
-- 'event_assignees'") ao criar um evento.
--
-- Causa provável: o script `db:repair-history` do package.json marca as
-- migrations 01–07 como aplicadas sem executá-las — feito para adotar um banco
-- criado à mão, mas que passa por cima de qualquer parte que não tenha rodado.
--
-- Reaplica o conteúdo da 07 de forma idempotente (a original já usava
-- `if not exists`), em vez de reescrever o histórico.
-- ============================================================
...
drop policy if exists "auth_full" on public.event_assignees;
...
drop policy if exists "auth_full" on public.event_attachments;
